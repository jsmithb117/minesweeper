...
describe('App', () => {
  ...
  it('should render 10 Piece components whose val is set as \'X\'', () => {
    expect(wrapper.findWhere(node => node?.props()?.piece?.val === 'X').length).toEqual(10);
  });
  it('should render 10 Piece components whose \'markedAsMine\' property is set to true', () => {
    expect(wrapper.findWhere(node => node?.props()?.piece?.markedAsMine === true).length).toEqual(10);
  });
  it('should render 20 Piece components with the \'uncovered\' property set', () => {
